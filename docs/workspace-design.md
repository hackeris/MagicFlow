...
| **W1.5 单入口化(2026-09-05 终版, 替代 W1 门户)** ✅(同日真机闭环) | 用户定谳:"环境管理是桌面时代虚构需求"(官方 workspace=云作用域, 本地后端恒单一)→ 门户整体撤出:环境列表/新建/环境名输入/workspaces.json 全删;首页=固定「ComfyUI 环境」卡 + 「启动 ComfyUI」按钮 + **4 步启动进度页**(准备环境→启动子进程→装载核心组件→后端就绪, 信号均为真: ensureStdlib 完成/startComfyChild 调用/system_stats 200; torch 装载期无后端信号 → 已用时长走秒), 就绪透出 ArkWeb | 打开 App:首页、**ps 无 comfy_child**(✅); 点启动→进度页(00:11 装载中/00:59 仍装载)→就绪→画布(✅ p1-p3 截图); 后端 0.34.0 就绪 50s; verify 驱导改「点启动」(✅ 全量回归); 退出不常驻=待验(App 杀后 ps, 若残留则 V2 加 NAPI kill) |
| **W1.6 官方同构启动页增强(2026-09-06)** ✅(同日真机验收) | 对照 Comfy-Desktop Brand Takeover(PR#1438/DeepWiki): ①品牌区(ComfyUI + 副标); ②**单调钳制进度条**(不回退, 段映射 0-40/45/48-92/100, ArkTS self-bar); ③装载期**叙事文案循环**(初始化运行时组件→注册模型与扩展→准备推理引擎→联机后端检查, 8s 轮换 — 官方"定时器+stdout 正则"同构); ④**可折叠日志终端**(diag.log+py-init.log 尾 60 行/1s 刷新, Scroll 滚动; ⚠ API24 ReadOptions 无 position → readTextSync 全读切尾); ⑤**学教训(用户实判)**: 字形符号 ▶/▼/✓ 会被字体渲染成 emoji 视效(不统一) → 一律 **SymbolGlyph 系统矢量**(chevron_down/right, checkmark); ⚠ uitest click 对 Row.onClick 不响应, 折叠头用 **Button 包裹**(2026-09-06 深度踩; 首页 Button 一直有效) | 已剪:展开/收起态截图(箭头/对勾矢量统一)、默认收起、实时日志(心跳+LIBFIND-HIT)、零自动启动 ps=0、clean 重开无自动子进程 |
...
1. 首屏=首页(单入口),**无自动后端**(`ps` 无 Native_libcomfy_child); — W1.5 终版: 首页=「启动 ComfyUI」按钮
2. 点启动 → 后端起 → 进度页 4 步(真实阶段+已用时长)→ 8188 画布就绪;模型缺时「缺模型条目」UI 触发下载(进度可见);
3. 再次打开 → 后端重启(非"秒开"): 保持零常驻语义(启动成本=重设施 1-2 分钟, 画面有明确进度);
