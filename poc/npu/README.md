...
本文档是全部 NPU 验证工作的最终结论(2026-09-07 首轮 + 2026-09-11 复跑与补验,均在 9030 设备)。
...
3. **形态障碍比预想小**:9030 设备上,原生子进程不仅能枚举到 NPU 设备(数量为 1,带真实硬件名),
   还能**完整执行** —— 构图、编译(Build)、执行(RunSync)全链通过、数值正确(2026-09-11 补验,
   此前只验到"可见")。主项目当前"子进程跑推理"的产品形态在 9030 目标机上不需要搬家。
...
| 子进程 | 官方原生子进程(与主项目推理子进程同一形态)能否看到**并执行** | **9030 上可以**:数量 1 带真实硬件名,ADD 单算子构图→编译→执行全链 rc=0、输出 2.00 正确(2026-09-11 补验执行链)。另一台设备(9020)上不可见——子进程可用性与设备有关,不是"所有子进程都不行" |
...
每次完整报告:设备 `filesDir/npu-poc-report.md`(与页面同内容);CH-06 子进程结果另落
`filesDir/npu-child.log`。2026-09-11 复跑:33/34 PASS 与首轮逐项一致,CH-06 升级为全链后
PASS(`build=0 run=0 out[0]=2.00`)。
