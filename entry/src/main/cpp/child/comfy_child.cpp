...
static void dlopen_python_check(const char *entryParams)
{
    ...
    if (Py_IsInitialized && Py_Initialize) {
        ...
        if (Py_IsInitialized() && PyRun_SimpleString) {
            // 启动自诊断(非测试): stdlib 可达性签名(site path 落 diag.log; smoke 判据不依赖此处)
            int r = PyRun_SimpleString("import os, site; print('PYROOT path0=', sys.path[0])");
            LOGI("PyRun_SimpleString rc=%{public}d", r);
            DIAG("PyRun import.os/site rc=%d", r);
        }
    } else {
    ...
    if (Py_IsInitialized()) {
        ...
        // 产品路径(2026-09-05 零探针化): torch import 的正确性/性能判据全部外置到
        //   ComfyUI 侧自检节点 OHOS_SmokeBench(见 custom_nodes/ohos_smoke + docs/smoke-design.md),
        //   smoke 走 HTTP 黑盒; 此处仅保留产物环境配置(线程/内核, 生产必需)。
    }
