                ...
                for f in files:
                    ...
                    if arc == "comfyui/comfy/utils.py":
                        ...
                        _probe = ('\ntry:\n'
                                  ...
                                  '    _cfg = getattr(_t, "__config__", None)\n'
                                  '    _cfg_txt = _cfg.show() if _cfg else ""\n'
                                  '    _pargs = []\n'
                                  '    try:\n'
                                  '        _pargs = _cfg.parallel_args()\n'
                                  '    except BaseException:\n'
                                  '        pass\n'
                                  '    _aff = ",".join(str(x) for x in _po.sched_getaffinity(0))\n'
                                  '    import ctypes as _ct\n'
                                  '    _lmp = None\n'
                                  '    for _ln in ("libomp.so", "libomp"):\n'
                                  '        try:\n'
                                  '            _lmp = _ct.CDLL(_ln)\n'
                                  '            break\n'
                                  '        except BaseException:\n'
                                  '            _lmp = None\n'
                                  '    if _lmp is not None:\n'
                                  '        _maxv = _lmp.omp_get_max_threads()\n'
                                  '        _numv = _lmp.omp_get_num_threads()\n'
                                  '        _proc = _lmp.omp_get_num_procs()\n'
                                  '    else:\n'
                                  '        _maxv = _numv = _proc = -1\n'
                                  # 切分实验: 改 env(6) + set_num_threads(6) 后重测 ——
                                  #   若 max 变 6 → libomp 动态(时序/init 错位); 若恒 1 → 库被编译钳制
                                  '    _mid = None\n'
                                  '    _tsz = -1\n'
                                  '    # run103 定谳(2026-09-04): 线程池 init=1 系 libomp 早期快照时序\n'
                                  '    #  (env 后改无效); torch.set_num_threads(12) 实测有效且不崩\n'
                                  '    #  (崩者是其 set_num_interop_threads)。正式解锁: 恒执行。\n'
                                  '    try:\n'
                                  '        _t.set_num_threads(12)\n'
                                  '        _tsz = _t.get_num_threads()\n'
                                  '    except BaseException as _tsz_e:\n'
                                  '        _tsz = -2\n'
                                  # run104: op 层到底并行否 —— matmul 4 连基准(纯 torch,无模型依赖)
                                  #   n=12 时 4x2048³ 若并行(<1s) vs 单核(~9-12s) → 一测分晓。
                                  '    _mmv = -99.0\n'
                                  '    try:\n'
                                  '        import time as _tw\n'
                                  '        _a = _t.randn(2048, 2048); _b = _t.randn(2048, 2048)\n'
                                  '        _t.mm(_a, _b)  # 预热\n'
                                  '        _s0 = _tw.time()\n'
                                  '        for _k in range(4):\n'
                                  '            _t.mm(_a, _b)\n'
                                  ...
                                  '        _pf.write("n=%d interop=%d cpu_count=%d affinity=%s\\n" % (\n'
                                  '            _bn, _bi, _po.cpu_count(), _aff))\n'
                                  '        _pf.write("env_OMP=%r env_LIMIT=%r\\n" % (\n'
                                  '            _po.environ.get("OMP_NUM_THREADS"),\n'
                                  '            _po.environ.get("OMP_THREAD_LIMIT")))\n'
                                  '        _pf.write("OMP-DIRECT max=%d num=%d procs=%d | set12 via torch=%d | MM4x=%.3fs\\n" % (\n'
                                  '            _maxv, _numv, _proc, _tsz, _mmv))\n'
                                  '        _pf.write("CONFIG-FULL:\\n" + str(_cfg_txt) + "\\n")\n'
